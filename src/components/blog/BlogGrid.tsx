          ...
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-12">
            <div className="max-w-2xl">
              ...
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-navy leading-[1.05]">
                <span data-cms-field="blogPage.grid.titleA" suppressContentEditableWarning>{t("blogPage.grid.titleA")}</span>{" "}
                <span className="text-gradient-gold" data-cms-field="blogPage.grid.titleB" suppressContentEditableWarning>{t("blogPage.grid.titleB")}</span>
              </h2>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {categories.map((category) => (
                ...
                >
                  {category === "All"
                    ? t("blogPage.grid.categories.All", { defaultValue: "All" })
                    : t(`blogPage.grid.categories.${category}`, { defaultValue: category })}
                </button>
              ))}
            </div>
          </div>
        ...
        {hasMore && (
          <div className="mt-12 text-center">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-6 py-3 text-sm font-semibold text-navy hover:border-gold hover:text-gold transition-colors"
            >
              {t("blogPage.grid.loadMore", { defaultValue: "Load more articles" })}
            </button>
          </div>
        )}
      </div>
        ...
        <div className="flex-1 flex flex-col p-6">
          <div className="flex items-center gap-3 text-[11px] text-muted-foreground mb-3">
            ...
            <span className="inline-flex items-center gap-1">
              <Clock className="h-3 w-3" /> {post.readMins}{" "}
              <span data-cms-field="blogPage.grid.min" suppressContentEditableWarning>{t("blogPage.grid.min")}</span>
            </span>
          </div>
