...
export function TeamAccessManager({ currentUser }: { currentUser: CurrentUser }) {
  ...
  return <>
    ...
    {creating && <CreateUserModal roles={roles} onClose={() => setCreating(false)} onCreated={async () => { setCreating(false); setNotice("Team member created. Their first sign-in will require a new private password."); await load(); }} />}
...
function CreateUserModal({ roles, onClose, onCreated }: { roles: Role[]; onClose: () => void; onCreated: () => Promise<void> }) {
  const [form, setForm] = useState({ full_name: "", email: "", password: "", role: "editor", is_active: true, require_password_change: true });
  ...
  return <div className="team-modal-backdrop" onMouseDown={onClose}><form className="team-create-modal" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}>
    <header><div><p className="eyebrow">New dashboard account</p><h2>Add team member</h2><span>Set their identity, temporary access and exact responsibility.</span></div><button type="button" onClick={onClose}><X size={19} /></button></header>
    <div className="team-modal-body">
      <section className="team-form-section"><div className="team-section-title"><i>1</i><span><b>Account identity</b><small>Use the team member&apos;s work information.</small></span></div><div className="team-form-grid"><label>Full name<input autoFocus value={form.full_name} onChange={(event) => setForm({ ...form, full_name: event.target.value })} required minLength={2} /></label><label>Work email<input type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} required autoComplete="off" /></label><label className="full">Temporary password<input type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} required minLength={12} autoComplete="new-password" /><small>Use at least 12 characters. This password is never displayed again.</small></label></div></section>
      ...
      <section className="team-startup-options"><label><input type="checkbox" checked={form.require_password_change} onChange={(event) => setForm({ ...form, require_password_change: event.target.checked })} /><span><b>Require password change at first sign-in</b><small>Recommended: the temporary password becomes unusable after they choose a private one.</small></span></label><label><input type="checkbox" checked={form.is_active} onChange={(event) => setForm({ ...form, is_active: event.target.checked })} /><span><b>Activate account now</b><small>Turn this off to prepare access without allowing sign-in yet.</small></span></label></section>
