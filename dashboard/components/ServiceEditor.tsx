...
export function ServiceEditor({ item, onClose, onPublished }: { item: ServiceItem; onClose: () => void; onPublished: () => void }) {
  ...
  return createPortal(<div className="modal-backdrop project-editor-backdrop" onMouseDown={close}>
    <section className="project-editor service-editor" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <footer className="project-editor-foot"><p>{notice || "Your changes are only public after Publish."}</p><div><button onClick={close}>Close</button><button className="editor-draft-button" onClick={() => void save("draft")} disabled={!form || !!saving}>Save draft</button><button className="primary-button compact" onClick={() => void save("publish")} disabled={!form || !!saving}>Publish</button></div></footer>
